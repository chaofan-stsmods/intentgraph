...
public class IconGroupEditorCanvasTool extends EditorCanvasTool {
    private final EditorCanvasDragHandler dragHandler;

    public IconGroupEditorCanvasTool(EditorCanvas canvas) {
        super(canvas);
    ...
    public void update() {
        ...
        if (InputHelper.justClickedLeft && canvas.mouseInCanvas()) {
            ...
            if (!this.selectedItems.isEmpty() || this.hoveredItem != null) {
                ...
                this.insertItem(graphDetail.iconGroups, (x, y) -> {
                    EditableIconGroup group = new EditableIconGroup(canvas.getGraphRenderX(), canvas.getGraphRenderY());
                    group.x = x - 0.5f;
                    group.y = y - 0.5f;
                    group.w = 1;
                    group.h = 1;
